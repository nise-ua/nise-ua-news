...
const SCENARIO_OPTIONS = [
  {
    ...
    description: 'Саркастичний, неформальний, скептичний авторський тон. Системний промпт Фази A береться з prompt.md.',
  },
  {
    id: 'architect',
    name: 'Архітектор',
    subtitle: 'серйозний · ще не використовувався',
    promptFile: 'prompt_deep.md',
    description: 'Холодний інтелект, техно-філософія, метод «від’їжджаючої камери». Системний промпт Фази A береться з prompt_deep.md.',
  },
];
  ...
  return {
    ...
    queue: {
      ...
      checkIntervalMs: {
        ...
        editable: true,
        note: 'Застосовується після перезапуску сервера',
      },
    },
...
function validatePatch(body) {
  ...
  if (body.llmVendor !== undefined) {
    ...
    if (typeof v !== 'string' || !LLM_VENDORS.includes(v)) {
      errors.push(`llmVendor: має бути одним із ${LLM_VENDORS.join(', ')}`);
    } else {
  ...
  const baseUrlField = (name, envKey) => {
    ...
    if (v.length > 300) {
      errors.push(`${name}: занадто довгий рядок (макс. 300 символів)`);
      return;
    }
    if (/[\n\r]/.test(v)) {
      errors.push(`${name}: не повинно містити переносів рядків`);
      return;
    }
    if (v.length > 0 && !/^https?:\/\//.test(v)) {
      errors.push(`${name}: має починатися з http:// або https://`);
      return;
    }
    env[envKey] = v;
  };
  ...
  const intField = (name, min, max) => {
    ...
    const v = body[name];
    if (!isInt(v)) {
      errors.push(`${name}: має бути цілим числом`);
    } else if (v < min || v > max) {
      errors.push(`${name}: має бути в діапазоні ${min}..${max}`);
    } else {
  ...
  const textField = (name, targetPath) => {
    ...
    if (typeof text !== 'string') {
      errors.push(`${name}.text: має бути рядком`);
    } else if (Buffer.byteLength(text, 'utf-8') > MAX_TEXT_BYTES) {
      errors.push(`${name}.text: занадто великий (макс. 100KB)`);
    } else {
...
router.patch('/', (req, res) => {
  ...
  if (Object.keys(env).length === 0 && files.length === 0) {
    return res.status(400).json({ error: 'Немає допустимих полів для зміни' });
  }
